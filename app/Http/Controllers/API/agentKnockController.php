...
class agentKnockController extends Controller
{
    ...
    public function reject(Request $req,$id)
    {
        ...
        if($knockrecord!=null)
        {
            ...
            $knock_update=AgentKnock::where('id',$record->id)->update($knock_data);
            if($knock_update==1)
            {
    ...
    {
        $knocks=AgentKnock::where('seller_id',$id)->where('isActive',1)->where('isApproved',0)->get()->toarray();
        if(!empty($knocks))
        {
